...
def upgrade():
    ...
    op.create_check_constraint(
        "ck_red_ml_non_negative", "global_inventory", sa.text("red_ml >= 0")
    )
    op.create_check_constraint(
        "ck_green_ml_non_negative", "global_inventory", sa.text("green_ml >= 0")
    )
    op.create_check_constraint(
        "ck_blue_ml_non_negative", "global_inventory", sa.text("blue_ml >= 0")
    )
    op.create_check_constraint(
        "ck_red_potions_non_negative", "global_inventory", sa.text("red_potions >= 0")
    )
    op.create_check_constraint(
        "ck_green_potions_non_negative",
        "global_inventory",
        sa.text("green_potions >= 0"),
    )
    op.create_check_constraint(
        "ck_blue_potions_non_negative", "global_inventory", sa.text("blue_potions >= 0")
    )
